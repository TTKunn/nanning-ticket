...
public interface RuleMapper extends BaseMapper<Rule> {
     ...
     * @return 有效记录数
     */
    @Select("SELECT COUNT(*) FROM rule WHERE scenic_id = #{scenicId} AND code = #{code} AND deleted_at IS NULL")
    ...
    /**
     * 统计指定园区下的有效规则数（用于回写 scenic.rule_count）
     *
     * @param scenicId 园区 ID
     * @return 有效规则数
     */
    @Select("SELECT COUNT(*) FROM rule WHERE scenic_id = #{scenicId} AND deleted_at IS NULL")
